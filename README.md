...
- **Registration & login** issue a signed JWT ([`auth.routes.ts`](src/routes/auth.routes.ts)). Passwords are hashed with bcrypt and never returned or logged.
...
- **Route guards** ([`auth.guards.ts`](src/guards/auth.guards.ts)) — `authenticate` verifies the JWT and `requireRole("admin")` enforces role-based access. Guards are declared per-route as Fastify `preHandler`s, so protection is visible right next to the route it protects.
...
Brings up Postgres and the app, running pending migrations first:
